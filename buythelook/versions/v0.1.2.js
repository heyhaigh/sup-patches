...
async function findProduct(item) {
  ...
  // Try primary search
  const primaryPrompt = [
    `Find a "${item.searchQuery}" for sale on Amazon.`,
    "",
    "Return the Amazon product page URL.",
    "If not on Amazon, try: Target, Walmart, ASOS, or any major retailer.",
    "Just find ONE real product that's similar - doesn't need to be exact match."
  ].join("\n");

  let result = await sup.ai.prompt(primaryPrompt, {
    useWebSearch: true,
    schema: productSearchSchema
  });

  // If no URL found, try fallback with simpler search
  if (!result.productUrl) {
    const fallbackPrompt = [
      `Search: "${item.color} ${item.category}" buy online`,
      "",
      "Find ANY product for sale that matches.",
      "Return the product page URL from any retailer."
    ].join("\n");

    result = await sup.ai.prompt(fallbackPrompt, {
      useWebSearch: true,
      schema: productSearchSchema
    });
  }
