...
async function analyzeLook(image, textContext) {
  const prompt = [
    ...
    "For each item, provide details that would help find a similar product to purchase.",
    ...
    "Be specific about colors, materials, and style details."
  ].join("\n");
...
async function findProduct(item) {
  const searchPrompt = [
    `Find a real product for sale online that matches this description:`,
    ...
    "",
    "Find ONE specific product currently for sale from a reputable retailer.",
    "Provide the exact product URL where someone can purchase it.",
    "Prefer well-known retailers (Nordstrom, ASOS, Zara, H&M, Amazon, etc.).",
    "The product should closely match the described item."
  ].join("\n");
