...
// Schema for finding a single product URL
const productUrlSchema = {
  schema: {
    productUrl: {
      type: "string",
      description: "Direct URL to a specific product page (not a search results page). Must be a full URL starting with https://"
    },
    productName: {
      type: "string",
      description: "The actual name of the product found"
    },
    retailer: {
      type: "string",
      description: "Name of the retailer (Amazon, Nordstrom, etc.)"
    },
    price: {
      type: "string",
...
async function findProductUrl(item) {
  const searchTerm = item.searchQuery || `${item.color} ${item.productName}`;
  const brandSearch = item.brand ? `${item.brand} ${searchTerm}` : searchTerm;

  const prompt = [
    `Find a SPECIFIC product for sale online that matches: "${brandSearch}"`,
    "",
    "Requirements:",
    "- Return a DIRECT product page URL (not search results)",
    "- Prefer: Amazon, Nordstrom, SSENSE, Farfetch, Net-a-Porter, ASOS",
    "- The URL must go directly to a buyable product",
    "- Find something visually similar in color and style",
    "",
    "Example good URL: https://www.amazon.com/dp/B08XYZ123",
    "Example bad URL: https://www.amazon.com/s?k=green+sweater (this is search results)"
  ].join("\n");
  ...
  try {
    ...
    if (result && result.productUrl && result.productUrl.startsWith("http")) {
      return {
        name: item.brand ? `${item.brand} ${item.productName}` : item.productName,
        originalPrice: item.price,
        foundPrice: result.price || null,
        url: result.productUrl,
        retailer: result.retailer || "Shop",
        foundProduct: result.productName || null
      };
    }
  } catch (e) {
    console.log(`Couldn't find product for: ${item.productName}`);
  }
  ...
  return {
    ...
    originalPrice: item.price,
    foundPrice: null,
    url: `https://www.google.com/search?tbm=shop&q=${encodeURIComponent(brandSearch)}`,
    retailer: "Google Shopping",
    foundProduct: null
  };
}

async function generateBuyLinks(schemaData) {
  sup.status("Finding products to buy...");
  const links = [];

  for (let i = 0; i < schemaData.items.length; i++) {
    const item = schemaData.items[i];
    sup.status(`Finding product ${i + 1}/${schemaData.items.length}...`);
    const link = await findProductUrl(item);
    links.push(link);
  }
...
async function main() {
  if (!sup.input.text && sup.input.images.length === 0) {
    return [
      ...
      "",
      "I'll create a 'Steal The Look' meme AND find real products you can actually buy.",
      `Version: ${VERSION}`
    ];
  }
  ...
  // Step 3: Find actual products to buy (async web search)
  const buyLinks = await generateBuyLinks(schemaData);
  ...
  for (const link of buyLinks) {
    const priceDisplay = link.foundPrice ? link.foundPrice : link.originalPrice;
    const productNote = link.foundProduct ? ` (${link.foundProduct})` : "";
    response.push(`• **${link.name}** ${priceDisplay}${productNote}`);
