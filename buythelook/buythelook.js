...
const schema = {
  schema: {
    title: {
      type: "string",
      description: "The header text like 'BUY HIS LOOK!!' or 'BUY HER LOOK!!' or 'BUY THEIR LOOK!!' with appropriate pronouns."
    },
    subjectName: {
      type: "string",
    ...
    items: {
      ...
      items: {
        ...
        properties: {
          brand: {
            type: "string",
            description: "Brand name. Luxury: Gucci, Prada, Balenciaga, Louis Vuitton, etc. Budget: Nike, Adidas, H&M, Uniqlo, Target, Amazon. Empty string for generic/unbranded."
          },
          productName: {
            type: "string",
            description: "Retail product name (e.g., 'Henley Shirt', 'Joggers', 'Chelsea Boots'). Use retail terms, not costume terms."
          },
          price: {
            type: "string",
            description: "Price with currency symbol (e.g., '$2,550', '€1,890', '$45')."
          },
          color: {
            type: "string",
            description: "EXACT color from the subject's outfit (e.g., 'white' for Link's tights, 'green' for his tunic, 'brown' for his boots)."
          },
          matchReason: {
            type: "string",
          ...
          category: {
            type: "string",
            description: "Product category for routing.",
            enum: ["clothing", "shoes", "accessories", "costume", "fanMerch", "athletic", "formal", "outdoor"]
          },
          searchQuery: {
            type: "string",
            description: "Retail search terms (e.g., 'white joggers', 'green henley'). Fan merch can use character refs."
          }
        }
      }
    },
...
const analysisPrompt = [
  "Create 'Buy The Look' fashion meme matching subject's outfit to real menswear.",
  "",
  "Rules:",
  ...
  "- Set category for each item (use 'fanMerch' for props)."
].join("\n");

const defaultsPrompt = [
  "Create a 'Buy The Look' meme for a random iconic subject.",
  "Options: Shrek, Link, Mario, Kirby, SpongeBob, a bird species, an animal, or video game character.",
  "Match 4-6 menswear items to the subject's colors (60% luxury, 40% budget/unbranded).",
  "Include one absurd but visually matching item."
].join("\n");
...
const imagePrompt = [
  "Create a 'BUY THE LOOK' fashion meme image assembled from the provided JSON data.",
  "",
  ...
  "- Large bold header at TOP: The 'title' field (e.g., 'BUY HIS LOOK!!' or 'BUY HER LOOK!!').",
  ...
  "",
  "The final image should look like the viral 'buy the look' memes shared on social media,",
...
const RELIABLE_STORE_URLS = {
  ...
  // Note: Removed women-focused stores (Victoria's Secret, Aerie, Torrid, Lane Bryant, etc.)
  // User base is predominantly male - keeping unisex/male-friendly stores only
...
// Keywords to detect product categories for smarter routing
const PRODUCT_CATEGORIES = {
  costume: ["costume", "cosplay", "elf", "fairy", "wizard", "witch", "superhero", "character", "halloween", "dress up", "roleplay", "link", "zelda", "mario", "pokemon", "anime", "disney", "marvel", "dc"],
  fanMerch: ["shield", "sword", "wand", "cape", "mask", "ears", "tail", "wings", "crown", "tiara", "replica", "prop", "plush", "funko", "collectible", "merch", "fandom"],
  athletic: ["sports", "athletic", "workout", "gym", "running", "yoga", "leggings", "sports bra", "sneakers", "trainers", "joggers", "sweatpants", "hoodie", "activewear"],
  formal: ["suit", "blazer", "dress shirt", "tie", "cufflinks", "formal", "tuxedo", "gown", "evening"],
  outdoor: ["hiking", "camping", "outdoor", "waterproof", "rain", "snow", "fleece", "parka"],
  vintage: ["vintage", "retro", "thrift", "secondhand", "antique", "classic", "80s", "90s", "70s"],
  jewelry: ["necklace", "bracelet", "ring", "earrings", "jewelry", "pendant", "chain", "anklet", "brooch"],
  accessories: ["hat", "cap", "beanie", "scarf", "gloves", "belt", "sunglasses", "glasses", "watch", "bag", "purse", "backpack", "wallet"],
  shoes: ["boots", "shoes", "loafers", "oxfords", "sneakers", "sandals", "slippers", "moccasins", "brogues", "derbys"],
  clothing: ["shirt", "pants", "trousers", "jeans", "shorts", "sweater", "cardigan", "jacket", "coat", "vest", "polo", "tee", "t-shirt", "chinos", "khakis"]
};

// Detect category - use schema category if provided, else keyword detection
function detectCategory(productName, searchQuery, schemaCategory) {
  // Use schema-provided category if valid
  if (schemaCategory && ["clothing", "shoes", "accessories", "costume", "fanMerch", "athletic", "formal", "outdoor"].includes(schemaCategory)) {
    return schemaCategory;
  }

  const text = `${productName} ${searchQuery}`.toLowerCase();
...
// Smart routing for generic/unbranded items based on product category
// Note: Using Amazon instead of Etsy because Etsy aggressively blocks bot-like traffic
function getSmartRetailer(category, query) {
  switch (category) {
    case "clothing":
      return {
        url: `https://www.amazon.com/s?k=${encodeURIComponent("mens " + query)}&rh=n:7141123011`,
        retailer: "Amazon Fashion"
      };
    case "shoes":
      return {
        url: `https://www.zappos.com/search?term=${encodeURIComponent("mens " + query)}`,
        retailer: "Zappos"
      };
    case "costume":
      return {
        url: `https://www.amazon.com/s?k=${encodeURIComponent(query + " costume mens")}`,
        retailer: "Amazon"
      };
    case "fanMerch":
      return {
    ...
    case "athletic":
      return {
        url: `https://www.amazon.com/s?k=${encodeURIComponent("mens " + query)}&rh=n:7141123011`,
        retailer: "Amazon Sports"
      };
    case "formal":
      return {
        url: `https://www.nordstrom.com/sr?keyword=${encodeURIComponent("mens " + query)}`,
        retailer: "Nordstrom"
      };
    case "outdoor":
      return {
        url: `https://www.rei.com/search?q=${encodeURIComponent(query)}`,
        retailer: "REI"
      };
    case "vintage":
      return {
        url: `https://www.amazon.com/s?k=${encodeURIComponent("mens " + query + " vintage")}`,
        retailer: "Amazon"
      };
    case "jewelry":
      return {
        url: `https://www.amazon.com/s?k=${encodeURIComponent("mens " + query)}`,
        retailer: "Amazon"
      };
    case "accessories":
      return {
...
// Add category context to search query for better results
function enhanceSearchQuery(query, productName, category) {
  const queryLower = query.toLowerCase();
  const productLower = productName.toLowerCase();

  // Don't add redundant terms - only enhance where it helps
  const enhancements = {
    costume: ["costume", "cosplay"],
    fanMerch: ["replica", "prop"],
  ...
  const terms = enhancements[category] || [];
  for (const term of terms) {
    if (!queryLower.includes(term) && !productLower.includes(term)) {
      return `${query} ${term}`;
    }
  }
  return query;
...
// Check if product is likely clothing (needs "mens" prefix)
function isClothingItem(productName) {
  const clothingKeywords = [
    "shirt", "pants", "trousers", "jeans", "shorts", "sweater", "hoodie", "jacket",
    "coat", "blazer", "suit", "tie", "vest", "cardigan", "polo", "tee", "t-shirt",
    "boots", "shoes", "sneakers", "loafers", "oxfords", "sandals", "slippers",
    "socks", "underwear", "boxers", "briefs", "tights", "leggings", "joggers",
    "sweatpants", "chinos", "khakis", "dress", "tunic", "robe", "pajamas",
    "belt", "scarf", "gloves", "hat", "cap", "beanie"
  ];
  const lower = productName.toLowerCase();
  return clothingKeywords.some(kw => lower.includes(kw));
}
...
function generateBuyLinks(schemaData) {
  ...
  for (const item of schemaData.items) {
    const baseQuery = item.searchQuery || `${item.color} ${item.productName}`;
    const brandLower = (item.brand || "").toLowerCase().trim();
    const category = detectCategory(item.productName, baseQuery, item.category);
    const query = enhanceSearchQuery(baseQuery, item.productName, category);

    // Add "mens" prefix for clothing items to avoid women's results
    const needsMens = isClothingItem(item.productName);
    const mensQuery = needsMens ? `mens ${query}` : query;
    const fullQuery = item.brand ? `${item.brand} ${mensQuery}` : mensQuery;

    let url;
    let retailer;

    if (brandLower && RELIABLE_STORE_URLS[brandLower]) {
      // Known reliable store - use direct search URL with mens prefix for clothing
      url = RELIABLE_STORE_URLS[brandLower] + encodeURIComponent(mensQuery);
      retailer = item.brand;
    } else if (brandLower) {
      ...
      // Generic/unbranded - smart routing based on product category
      const smartRoute = getSmartRetailer(category, query);
      url = smartRoute.url;
      retailer = smartRoute.retailer;
    }
...
function main() {
  // Check for reroll action
  const isReroll = sup.message.get("reroll");
  if (isReroll) {
    sup.message.set("reroll", null);
    // Fall through to regenerate using stored input
  }

  // Show welcome if no input and not a reroll
  const hasInput = sup.input.text || sup.input.images.length > 0;
  const hasStoredInput = sup.message.get("lastInput");

  if (!hasInput && !isReroll) {
    return [
      "🛍️ Buy The Look",
      "",
      "Send an image or describe a character/person/thing!",
      "",
      "I'll create a 'Buy The Look' meme AND give you links to actually buy the items.",
      `Version: ${VERSION}`
    ];
  }

  // Store input for potential reroll (only on new input)
  if (hasInput) {
...
// Reroll callback - triggers regeneration
function onReroll() {
