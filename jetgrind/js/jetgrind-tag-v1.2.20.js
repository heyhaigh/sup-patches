...
function renderTagPortfolioView() {
  ...
  if (portfolio.length === 0) {
    ...
    // TEST 6: Debug - what IS tag.image?
    const firstTag = portfolio[0];
    const imageType = typeof firstTag.image;
    const imageValue = String(firstTag.image).substring(0, 200); // First 200 chars
    const gridHtml = `<div style="padding:20px;background:white;font-family:monospace;font-size:11px;">
      <h3>Debug: tag.image</h3>
      <p><b>Type:</b> ${imageType}</p>
      <p><b>Value (first 200 chars):</b></p>
      <p style="word-break:break-all;background:#eee;padding:8px;">${imageValue}</p>
    </div>`;
