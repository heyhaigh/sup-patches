...
function onSubmitTag() {
  const text = sup.get("inputText") || "";
  const preset = sup.get("selectedPreset") || sup.user.get("favoriteStyle") || "random";

  if (!text || text.trim() === "") return;
  if (text.trim().length > MAX_LENGTH) return;

  doGenerateTag(text, preset);
}

function onInputChange(value) {
  sup.set("inputText", value);
}

function onPresetSelect(key) {
  sup.set("selectedPreset", key);
}
...
function renderFormView() {
  const currentText = sup.get("inputText") || "";
  const favoriteStyle = sup.user.get("favoriteStyle") || "random";
  const selectedPreset = sup.get("selectedPreset") || favoriteStyle;
  ...
  return [
    "🎨 JET GRIND TAG",
    sup.input("Enter your tag (1-3 words)...", onInputChange, currentText),
    `${charCount}/${MAX_LENGTH} characters${isOverLimit ? " - TOO LONG!" : ""}`,
    "\nStyle:",
    sup.button(selectedPreset === "random" ? "🎲 RANDOM ✓" : "🎲 Random", () => onPresetSelect("random")),
    sup.button(selectedPreset === "ggs" ? "⭐ GGs ✓" : "⭐ GGs", () => onPresetSelect("ggs")),
    sup.button(selectedPreset === "noisetanks" ? "⚡ Noise ✓" : "⚡ Noise", () => onPresetSelect("noisetanks")),
    sup.button(selectedPreset === "poisonjam" ? "💀 Poison ✓" : "💀 Poison", () => onPresetSelect("poisonjam")),
    sup.button(selectedPreset === "loveshockers" ? "💖 Love ✓" : "💖 Love", () => onPresetSelect("loveshockers")),
    "",
    isDisabled
      ? "Enter text to generate a tag"
      : sup.button("🎨 TAG!", onSubmitTag),
    "",
    sup.button("⭐ Favorites", onViewFavorites),
