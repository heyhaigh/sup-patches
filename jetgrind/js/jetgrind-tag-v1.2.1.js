...
function onClearTagPortfolio() {
  sup.message.set("view", "clearTagPortfolio");
}

function onConfirmClearTagPortfolio() {
  sup.user.set("tagPortfolio", []);
  sup.message.set("view", "tagPortfolio");
}

function onCancelClearTagPortfolio() {
  sup.message.set("view", "tagPortfolio");
}
...
function onClearPhotoJournal() {
  sup.message.set("view", "clearPhotoJournal");
}

function onConfirmClearPhotoJournal() {
  sup.user.set("photoJournal", []);
  sup.message.set("view", "photoJournal");
}

function onCancelClearPhotoJournal() {
  sup.message.set("view", "photoJournal");
}
...
function renderTagPortfolioView() {
  ...
  if (portfolio.length === 0) {
    return [
      ...
      "",
      "No tags saved yet!",
      "Generate a tag and save it here.",
      "",
      sup.button("📸 Photo Journal", onViewPhotoJournal)
    ];
  }
  ...
  const response = [
    `🏷️ Tag Portfolio (${portfolio.length}) - Page ${currentPage + 1}/${totalPages}`
  ];
  ...
  for (let i = startIdx; i >= endIdx; i--) {
    ...
    response.push(`"${tag.text}"${rareLabel}`);
  ...
  response.push(sup.button("📸 Photo Journal", onViewPhotoJournal));
  response.push(sup.button("🗑️ Clear All", onClearTagPortfolio));

  return response;
}

function renderClearTagPortfolioView() {
  return [
    "⚠️ Delete ALL saved tags from Tag Portfolio?",
    sup.button("✅ Yes, Delete All", onConfirmClearTagPortfolio),
    sup.button("❌ Cancel", onCancelClearTagPortfolio)
  ];
}
...
function renderPhotoJournalView() {
  ...
  if (journal.length === 0) {
    return [
      ...
      "",
      "No photos saved yet!",
      "Add a backdrop to a tag and save it here.",
      "",
      sup.button("🏷️ Tag Portfolio", onViewTagPortfolio)
    ];
  }
  ...
  const response = [
    `📸 Photo Journal (${journal.length}) - Page ${currentPage + 1}/${totalPages}`
  ];
  ...
  for (let i = startIdx; i >= endIdx; i--) {
    ...
    response.push(`"${photo.text}" on ${photo.backdropName}${rareLabel}`);
  ...
  response.push(sup.button("🏷️ Tag Portfolio", onViewTagPortfolio));
  response.push(sup.button("🗑️ Clear All", onClearPhotoJournal));

  return response;
}

function renderClearPhotoJournalView() {
  return [
    "⚠️ Delete ALL saved photos from Photo Journal?",
    sup.button("✅ Yes, Delete All", onConfirmClearPhotoJournal),
    sup.button("❌ Cancel", onCancelClearPhotoJournal)
  ];
}
...
async function main() {
  ...
  switch (view) {
    ...
    case "clearTagPortfolio":
      return renderClearTagPortfolioView();
    case "clearPhotoJournal":
      return renderClearPhotoJournalView();
    case "favorites":
