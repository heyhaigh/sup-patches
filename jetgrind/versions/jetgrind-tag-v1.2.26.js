...
// Styles that can randomly appear in generation (with percentage chances)
const RANDOM_STYLE_POOL = [
  { style: "grunge", chance: 10 },
  { style: "blackwork", chance: 10 }
];

function checkRandomStyle() {
  for (const entry of RANDOM_STYLE_POOL) {
...
function buildPrompt(text, presetKey = "random") {
  if (presetKey === "random") {
    ...
    // Check for random style variant (Grunge, Blackwork)
    const randomStyle = checkRandomStyle();
    if (randomStyle) {
      presetKey = randomStyle;
    }
  }
