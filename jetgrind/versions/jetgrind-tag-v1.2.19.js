...
function renderTagPortfolioView() {
  ...
  if (portfolio.length === 0) {
    ...
    // TEST 5: Include images in the grid
    const gridHtml = `<div style="padding:20px;background:white;">
      <h2>Your Tags</h2>
      ${portfolio.map((tag, idx) => `
        <div style="background:#eee;padding:8px;margin:4px;">
          <img src="${tag.image}" style="width:100%;max-height:60px;object-fit:contain;" />
          <div>${idx + 1}. ${tag.text || 'Unnamed'}</div>
        </div>
      `).join('')}
    </div>`;
    response.push(sup.html(gridHtml, { width: 400, height: 400, type: 'image' }));
