...
function renderTagPortfolioView() {
  ...
  if (portfolio.length === 0) {
    ...
    // TEST 7: Image grid using tag.image.url
    const gridHtml = `<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;padding:12px;background:white;">
      ${portfolio.map(tag => `
        <div style="background:#f5f5f5;border-radius:8px;overflow:hidden;">
          <img src="${tag.image.url}" style="width:100%;display:block;" />
        </div>
      `).join('')}
    </div>`;
    response.push(sup.html(gridHtml, { width: 600, height: 400, type: 'image' }));
