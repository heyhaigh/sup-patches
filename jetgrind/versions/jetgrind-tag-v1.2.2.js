...
function renderTagPortfolioView() {
  ...
  const response = [
    ...
    "Tap a tag to view full size"
  ];

  // 3-across grid of clickable thumbnails
  for (let i = startIdx; i >= endIdx; i--) {
    const tag = portfolio[i];
    const rareIcon = tag.rare ? "✨" : "";
    response.push(sup.button(`${rareIcon}"${tag.text}"`, () => onViewPortfolioItem(i)));
  }
...
function renderPhotoJournalView() {
  ...
  for (let i = startIdx; i >= endIdx; i--) {
    ...
    const rareIcon = photo.rare ? "✨" : "";
    response.push(sup.button(`${rareIcon}"${photo.text}" ${photo.backdropName}`, () => onViewJournalItem(i)));
  }
