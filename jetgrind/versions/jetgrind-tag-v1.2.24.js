...
function renderTagPortfolioView() {
  ...
  if (portfolio.length === 0) {
    ...
    // - Transparent background on container
    // - 16px rounded corners on thumbnails
    // - Vertically centered images
    const itemsToShow = [];
    for (let i = startIdx; i >= endIdx; i--) {
      itemsToShow.push(portfolio[i]);
    }
    // Calculate dynamic height based on number of rows
    // 3 columns, so rows = ceil(items / 3)
    // Each thumbnail is ~150px tall (width 500 / 3 cols - gaps), plus 8px gap between rows
    const numRows = Math.ceil(itemsToShow.length / 3);
    const thumbSize = 150;
    const gap = 8;
    const gridHeight = (numRows * thumbSize) + ((numRows - 1) * gap);

    // Transparent background with transparent: true option
    // White thumbnail backgrounds for clean look
    const gridHtml = `<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:${gap}px;">
      ${itemsToShow.map(tag => `
        <div style="display:flex;align-items:center;justify-content:center;background:#ffffff;border-radius:16px;overflow:hidden;aspect-ratio:1;">
          <img src="${tag.image.url}" style="width:100%;height:100%;object-fit:contain;" />
        </div>
      `).join('')}
    </div>`;
    response.push(sup.html(gridHtml, { width: 500, height: gridHeight, type: 'image', transparent: true }));

    // Just numbered edit buttons (no stacked images)
    for (let i = startIdx; i >= endIdx; i--) {
      const displayNum = startIdx - i + 1;
      response.push(sup.button(`Edit #${displayNum}`, () => onViewPortfolioItem(i)));
    }
...
function renderPhotoJournalView() {
  ...
  if (journal.length === 0) {
    ...
    // Build array of items to show
    const itemsToShow = [];
    for (let i = startIdx; i >= endIdx; i--) {
      itemsToShow.push({ photo: journal[i], index: i });
    }
    ...
    // Grid with transparent background and white thumbnails
    const gridHtml = `<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:${gap}px;">
      ${itemsToShow.map(item => `
        <div style="display:flex;align-items:center;justify-content:center;background:#ffffff;border-radius:16px;overflow:hidden;aspect-ratio:1;">
          <img src="${item.photo.image.url}" style="width:100%;height:100%;object-fit:contain;" />
        </div>
      `).join('')}
    </div>`;
    response.push(sup.html(gridHtml, { width: 500, height: gridHeight, type: 'image', transparent: true }));
    ...
    for (let i = 0; i < itemsToShow.length; i++) {
      ...
      const itemIndex = itemsToShow[i].index;
      response.push(sup.button(`Edit #${displayNum}`, () => onViewJournalItem(itemIndex)));
    }
