...
function renderTagPortfolioView() {
  ...
  if (portfolio.length === 0) {
    ...
    // TEST 4: Map over actual portfolio data (text only, no images)
    const gridHtml = `<div style="padding:20px;background:white;">
      <h2>Your Tags</h2>
      ${portfolio.map((tag, idx) => `<div style="background:#eee;padding:8px;margin:4px;">${idx + 1}. ${tag.text || 'Unnamed'}</div>`).join('')}
    </div>`;
    response.push(sup.html(gridHtml, { width: 300, height: 250, type: 'image' }));
