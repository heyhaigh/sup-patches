...
function renderTagPortfolioView() {
  ...
  if (portfolio.length === 0) {
    ...
    // TEST 3: Template literal with .map()
    const names = ['Tag 1', 'Tag 2', 'Tag 3'];
    const gridHtml = `<div style="padding:20px;background:white;">
      <h2>Grid Test</h2>
      ${names.map(n => `<div style="background:#eee;padding:8px;margin:4px;">${n}</div>`).join('')}
    </div>`;
    response.push(sup.html(gridHtml, { width: 300, height: 200, type: 'image' }));
